...
6. **After the post is published and merged**, delete the `.plan.md` file — it is a working artifact, not part of the published blog

If the user asks you to "just write it" or skip planning, remind them that the plan-first approach ensures quality. For short posts (announcements, link posts, < 300 words), offer to approve a compact one-paragraph outline inline instead of a full plan file, then proceed immediately on approval.
...
- Use standard MDX frontmatter (all fields are required):
  ```yaml
  ---
  title: "Post Title"
  ...
  tokenID: <integer or omit if not yet minted>
  ---
  ```
