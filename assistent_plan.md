...
- [~] **Verify B3 against real Base Sepolia — in progress, two real `sc_llm_x402.ts` bugs found and fixed, one blocking upstream SDK bug found (not ours to fix, workaround needed):**
  ...
  3. **Not ours to fix — real, reproducible bug in `@x402/evm` itself** (present in both 2.17.0, what we run, and 2.18.0, npm latest): `wrapFetchWithPayment` crashes client-side (`processSettleResponse`, `channelState.channelId.toLowerCase()` on `undefined`) immediately after a real, correctly-settled deposit, because the SDK's own server-side `handleEnrichSettlementResponse` never includes `channelId` in a deposit-type settlement response. Confirmed this is not avoidable via `x402HTTPClient.processResponse()` either (crash happens inside `wrapFetchWithPayment` itself, before either response-processing method runs) — tested directly against the official reference client/server example code, not just our own. Full repro + draft GitHub issue: `x402_facilitator/UPSTREAM_ISSUES.md`. **Blocks a clean B3 verification run and Phase C** until either upstream fixes it or we apply a local `patch-package` workaround (one-line defensive guard, not yet applied).
