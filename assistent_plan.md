...
- [ ] Verify B3 + B4 against real Base Sepolia (same rigor as Phase A: always check actual on-chain/storage state, not just `success: true`) — run the local handler against the buyer notebook's deposit+voucher flow, confirm `chargedCumulativeAmount` genuinely changes in S3 and matches `convertTokensToUsdcCost(LLM_ESTIMATED_TOKENS_PER_MESSAGE)` exactly; then run the cron once manually, confirm a real decoded `Claimed` event and `receivers()` bucket movement.
