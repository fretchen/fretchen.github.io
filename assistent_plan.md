...
**Phase B — Server (`scw_js/` + `shared/s3-utils/`)** — detailed KISS plan prepared (2026-07-15); implementation not yet started, paused pending explicit go-ahead. Full plan also at `~/.claude/plans/now-propose-a-concrete-majestic-patterson.md`.
...
- **Correction from SDK exploration:** the original plan's `setSettlementOverrides` reference was wrong — that's an `@x402/core` mechanism for the separate `upto` scheme, not batch-settlement. Batch-settlement's real "charge only the actual post-generation cost" mechanism is simpler: the SDK's `Channel` record has a `chargedCumulativeAmount` field the resource server controls directly — after computing real cost via the existing `convertTokensToCost()`, the handler just increments `chargedCumulativeAmount` in its own `ChannelStorage`, capped at what the client authorized. No override API needed.
...
- **`scw_js` gets its own `scw_js/notebooks/` folder** (Deno kernel, own scoped `deno.json`, same `nodeModulesDir: "auto"` + `lock: false` fix already proven in `x402_facilitator/notebooks/deno.json`) — mirrors the existing per-package convention (`growth-agent/notebooks/` is already its own thing). B0's spike belongs there, not in `x402_facilitator/notebooks/`, since it's testing scw_js's own future server behavior, not the facilitator.

**Steps:**
- [ ] **B0 — Spike (do FIRST, blocks B3 only):** in a new `scw_js/notebooks/` Deno notebook, register a **server**-side `BatchSettlementEvmScheme` (from `@x402/evm/batch-settlement/server`) with `InMemoryChannelStorage`, feed it a real deposit payload (reusing the buyer flow from `x402_facilitator/notebooks/x402_batch_settlement_buyer.ipynb`), call `resourceServer.verifyPayment(...)`, and inspect exactly what changed in storage. Repeat with a subsequent voucher payload. Confirms whether `verifyPayment()` alone updates `ChannelStorage` or whether the handler must also call `mergeRequestContext`/`rememberChannelSnapshot` itself, and confirms manually bumping `chargedCumulativeAmount` via `updateChannel` sticks. Record the confirmed sequence in `sc_llm_x402.ts`'s header comment (B3) so it's not re-derived later.
- [ ] **B1** Extend `@fretchen/s3-utils` (`shared/s3-utils/src/index.ts`, additive only): return ETag on GET (new `getS3ObjectWithMeta`); `ifMatch`/`ifNoneMatch` on PUT surfacing **412 as a status, not a throw**; add `listObjects(prefix)` + conditional delete. Rebuild before scw_js picks it up. Independent of B0 — just needs to satisfy the `ChannelStorage` interface's atomicity contract, regardless of how the handler calls it.
- [ ] **B2** New `scw_js/x402_channel_storage.ts` — S3-backed `ChannelStorage` (get/list/updateChannel): `channels/<id>.json`, **private ACL** (server-internal, not the public `merkle/` prefix pattern); `updateChannel` = GET+ETag → callback → conditional PUT (`ifNoneMatch:"*"` new / `ifMatch:<etag>` existing) → retry on 412 (bounded, ~3 attempts) → callback returning `undefined` → conditional DELETE; `list` = `listObjects("channels/")` + GET each (N+1, acceptable at this scale — YAGNI on optimizing further).
- [ ] **B3** New `scw_js/sc_llm_x402.ts` (parallel to `sc_llm.ts`, not a replacement) + `createLLMResourceServer()` in `x402_server.ts`: register server `BatchSettlementEvmScheme(receiver, { storage: s3ChannelStorage })`. Flow: `extractPaymentPayload` → `create402Response` if missing → `verifyPayment` → `callLLMAPI` (unchanged) → `convertTokensToCost` (unchanged) → **update `chargedCumulativeAmount` in storage** (no on-chain settle here — that's B4's cron) → respond. Keep `body.data.prompt` request shape. Remove bearer auth (`auth_utils`) + `checkWalletBalance` + merkle calls (`saveLeafToTree`/`processMerkleTree`/`startNewTree`) — these simply aren't imported into the new file. New `llmx402` function in `serverless.yml`, additive.
- [ ] **B4** New scheduled/cron serverless function reading the same S3 `ChannelStorage`: `scheme.createChannelManager(facilitatorClient, network).claimAndSettle()` on a fixed interval (start hardcoded, e.g. hourly — no configurability yet). New secret `RECEIVER_AUTHORIZER_PRIVATE_KEY` (self-managed receiver — facilitator has no authorizer key); needs a real (non-default) RPC endpoint.
- [ ] Tests per step: B1 mocked-fetch unit tests (conditional-write behavior already proven against real Scaleway in Phase A, not re-proven here); B2 CAS conflict/retry/list unit tests against mocked s3-utils; B3 unit tests mirroring `sc_llm.test.ts`'s style (402/verify-fail/charge-amount assertions); B4 wiring unit test (mocked channel manager).
- [ ] Verify B3 + B4 against real Base Sepolia (same rigor as Phase A: always check actual on-chain/storage state, not just `success: true`) — run the local handler against the buyer notebook's deposit+voucher flow, confirm `chargedCumulativeAmount` genuinely changes in S3; then run the cron once manually, confirm a real decoded `Claimed` event and `receivers()` bucket movement.
