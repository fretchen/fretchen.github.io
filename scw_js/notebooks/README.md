...
| `search_x402_buyer.ipynb` | Deno/TS | The same, for `search_api.ts`'s two paid routes — `GET /fetch` and `GET /search`. Scheme: `batch-settlement`, on the **chat's own channel** (same receiver, so a browser that has already chatted pays with a voucher and sees no wallet prompt). Covers what the unit tests mock away: that a GET can be paid for, that the 402 is readable by `@x402/fetch`, and that the second call opens no second channel. **Mainnet only — every run is real money.** |
