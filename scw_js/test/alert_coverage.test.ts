 ...
 * observability/alerts/payments.yaml's "The `logger.error` convention" section. The parsing logic
 * is shared with x402_facilitator/test/alert_coverage.test.ts via
 * `observability/alertCoverage.ts`; see that file's header for its documented limits.
 *
 * Two assertions:
    ...
    it(`"${message}" (${file}) is alertable`, () => {
      ...
      expect(
        ...
        `logger.error("${message}") in ${file} is not matched by any rule's first filter in ` +
          `observability/alerts/*.yaml, and is not in this test's EXEMPT map. Either add/extend a rule there, ` +
