...
im Code („oldest first to keep stable URLs") schützt nur davor, dass _neuere_ Posts die
...
- [x] Root `CLAUDE.md` → Abschnitt _Blog Posts_: `.tsx` als Format gestrichen
...
- `blog/merkle_ai_batching_fundamentals.mdx` hat einen neuen Abschnitt _"A note on the demos
  above"_ (vor dem Schlussabschnitt), der offenlegt, dass die Demos eine minimale, gegen das
...
Plans, sondern Vikes Routing-Modell: eine Seite _ist_ ein Verzeichnis mit einer
...
`pages/about+Page.js`-Syntax vor, nennt sie _"functional"_, später _"Neat ideas, I like
them … we're going in the right direction 👀"_. Seit über zwei Jahren nicht umgesetzt.
...
- ✅ **Echtes Code-Splitting pro Seite**, nachgewiesen: Seite A referenziert nur A's Chunk,
  ...
  _Pointer Imports_ um (alles, was kein „plain script file" ist) und splittet selbst.
...
- Ausgeliefert in 0.4.259 via [PR #3356](https://github.com/vikejs/vike/pull/3356) (wir sind
  ...
  seit 2022**.
- brillout: _"Programmatic at config time is easy and **definitely on the radar** (I'll
  actually personally need it for a couple of things I want to implement)."_ Der schwierige
...
### Warum Route-Funktionen das _nicht_ lösen
...
Route Function schickt also weiterhin alle Posts durch _ein_ Seitenmodul; Vike kann das
...
- [x] `CLAUDE.md` (Repo-Wurzel), Abschnitt _Blog Posts_: `.tsx` als Post-Format streichen (erledigt in Stufe 1, siehe oben)
