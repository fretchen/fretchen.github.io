...
**The rule: serif reads, sans operates** — decided by what the content _is_, not by which route
it sits on. Sans is the site-wide default (`globalCss`), because most surfaces are ones you
operate. Serif is _opted into_ by the `prose` **text style** in `panda.config.ts`, applied as
...
restyled, and the body serif is chosen to sit _beside_ it, not to match it. `katex.min.css` is
...
stacks. They remain valid tokens, so `fontFamily: "mono"` fails _silently_. Rule 5 in
...
the quantum notes: _"A simple circuit diagram. It shows the initial state, an entanglement gate,
a number of single qubit gates and the final readout."_ **If the prose already names the figure,
...
inline KaTeX — they have to sit _with_ the equations rather than against them. Do not set them in
...
**A figure may wear its page's territory hue — on the elements that _name_ things only.** Participant
...
- **Essay colours are deliberately outside the system.** The chart palettes in
  `components/blog/` encode _series identity_ (which actor, which asset) and are chosen for
...
- A handful of essay widgets set `borderRadius` in a JSX `style={{}}` because the value is
  computed at runtime. Inline styles are plain CSS and cannot take token _names_, so raw units
...
- **Diagram and widget containers still carry tinted washes**, predating the [Figures](#figures)
  rule and violating both _no tinted surfaces_ and _never write a hex in a component_:
