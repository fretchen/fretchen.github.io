...
In the [previous blog post](./11), I described how I set up an image generator that quite nicely fulfills my needs: The generator in the background is open-source, I have full cost control, I made the system easily usable to other and it is really privacy-conserving. However, I could not resist the temptation to make it easier for others to see some of the images and if try to set up small incentive systems in form of collector NFTs. The main changes are:
...
This is a super simple approach, which is inspired by the [robots.txt](https://en.wikipedia.org/wiki/Robots.txt) approach. It does not securely makes things private, but it unlists them from some public galleries. This approach is inspired by robots.txt - just as robots.txt doesn't technically prevent access but signals to web crawlers whether content should be indexed, the `isListed` flag signals whether an image should appear in public galleries. The data remains on the blockchain and technically accessible, but won't be displayed in the public interface.
...
This project demonstrates how blockchain technology can create sustainable creator economies for AI-generated art. The combination of public galleries, collector NFTs, and quadratic pricing creates interesting dynamics that traditional platforms can't replicate.
