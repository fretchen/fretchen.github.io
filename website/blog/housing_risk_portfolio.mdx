...
**Model:** Each asset's annual euro risk is its value times its volatility: `euroVol = value × σ`. The risk bar shows how much each asset contributes to total risk. The "share of risk" in the table is each asset's variance share: $(\text{weight} \times \sigma)^2 \,/\, \Sigma(\text{weight} \times \sigma)^2$, where weight = value / total assets.
