...
export const updatedLlmWorkflowParticipants = [
  { id: "alice", label: "Alice" },
  { id: "contract", label: "Smart Contract" },
  { id: "aiService", label: "AI Service" },
  { id: "batchCoord", label: "Batch Coordinator" },
  { id: "settlement", label: "Settlement Layer" },
];

export const updatedLlmWorkflowSteps = [
  { kind: "note", from: "alice", to: "contract", label: "Phase 1: Setup - Prepaid Deposit" },
  { kind: "message", from: "alice", to: "contract", label: "Deposit ETH for AI usage" },
  { kind: "note", from: "contract", to: "contract", label: "Update user balance" },
  { kind: "note", from: "alice", to: "settlement", label: "Phase 2: Usage - Instant AI Interactions" },
  { kind: "message", from: "alice", to: "aiService", label: 'Request: "Analyze sentiment"' },
  { kind: "message", from: "aiService", to: "contract", label: "Validate sufficient balance" },
  { kind: "note", from: "aiService", to: "aiService", label: "Process with LLM API" },
  { kind: "message", from: "aiService", to: "alice", label: "Instant AI response", style: "dashed" },
  { kind: "message", from: "aiService", to: "batchCoord", label: "Queue request for batching" },
  {
    kind: "note",
    from: "batchCoord",
    to: "settlement",
    label: "Phase 3: Settlement - Efficient Batch Processing",
  },
  { kind: "note", from: "batchCoord", to: "batchCoord", label: "Build Merkle tree from queued requests" },
  { kind: "message", from: "batchCoord", to: "contract", label: "Process batch settlement" },
  { kind: "note", from: "contract", to: "contract", label: "Verify proofs and deduct costs" },
  { kind: "message", from: "contract", to: "settlement", label: "Single efficient transaction" },
  { kind: "message", from: "settlement", to: "alice", label: "Settlement confirmation", style: "dashed" },
];
...
<SequenceDiagram
  participants={updatedLlmWorkflowParticipants}
  steps={updatedLlmWorkflowSteps}
  caption="Simplified User Journey: Prepaid AI with Batch Settlement"
/>
