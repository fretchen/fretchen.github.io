...
All of them have strong support by the official x402 SDKs. Under the hood, my facilitator uses the following packages from the x402 ecosystem:
...
Part of my website is the imagegen feature, which calls a serverless function to generate AI images via Black Forest Labs and mints an NFT as a certificate of authenticity via the GenImNFTv4 contract. This endpoint is now upgraded to use the x402 standard for payments and accessible to anyone interested under [imagegen-agent.fretchen.eu](https://imagegen-agent.fretchen.eu). So
...
Altogether, this is a nice step to simplify payments for AI services. The code in the frontend is now substantially easier and more standardized. More importantly, I can start searching for other AI agents with x402 payment interface and connect to the network.

And if I find a good way to introduce a fee structure for the facilitator, I can see x402 becoming a really powerful standard for pay-per-use AI services.
