...
export const x402FlowParticipants = [
  { id: "buyer", labelLines: ["fretchen.eu/imagegen", "(Buyer)"] },
  { id: "seller", labelLines: ["imagegen-agent.fretchen.eu", "(Seller)"] },
  { id: "facilitator", labelLines: ["facilitator.fretchen.eu", "(Facilitator)"] },
  { id: "blockchain", labelLines: ["Optimism L2", "(USDC)"] },
];

export const x402FlowSteps = [
  { kind: "message", from: "buyer", to: "seller", label: "1. POST /genimg (no payment)" },
  {
    kind: "message",
    from: "seller",
    to: "buyer",
    label: "2. 402 Payment Required + PAYMENT-REQUIRED header",
    style: "dashed",
  },
  { kind: "note", from: "buyer", to: "buyer", label: "3. User signs EIP-3009 payment authorization" },
  {
    kind: "message",
    from: "buyer",
    to: "seller",
    label: "4. POST /genimg + PAYMENT-SIGNATURE header",
  },
  { kind: "message", from: "seller", to: "facilitator", label: "5. POST /verify" },
  { kind: "message", from: "facilitator", to: "seller", label: "6. Payment valid ✓", style: "dashed" },
  { kind: "note", from: "seller", to: "seller", label: "7. Generate image + Mint NFT" },
  { kind: "message", from: "seller", to: "facilitator", label: "8. POST /settle" },
  { kind: "message", from: "facilitator", to: "blockchain", label: "9. transferWithAuthorization" },
  {
    kind: "message",
    from: "blockchain",
    to: "facilitator",
  ...
  },
  {
    ...
    from: "facilitator",
    to: "seller",
    label: "11. Settlement complete",
    style: "dashed",
  },
  { kind: "message", from: "seller", to: "buyer", label: "12. 200 OK + Image URL + NFT", style: "dashed" },
];
...
<SequenceDiagram participants={x402FlowParticipants} steps={x402FlowSteps} caption="x402 Payment Flow" />
