...
This response tells the client everything it needs: pay **0.07 USDC** (`amount: "70000"` with 6 decimals) on **Optimism Mainnet** (`network: "eip155:10"`) to the server wallet. The client can then sign an EIP-3009 authorization and resubmit the request with the `PAYMENT-SIGNATURE` header.

The key difference from my previous set-up is the facilitator. It handles all the blockchain complexity – verifying signatures off-chain and settling payments on-chain – so the seller doesn't need to maintain blockchain infrastructure.
