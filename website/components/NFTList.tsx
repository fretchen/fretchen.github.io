    ...
    <div className={styles.nftList.container}>
      ...
      <div className={styles.tabs.container}>
        <div className={styles.tabs.tabList}>
          <Tab
            label={isConnected ? `${useLocale({label: "imagegen.myArtworks"})} (${userBalance?.toString() || "0"})` : useLocale({label: "imagegen.myArtworks"})}
            isActive={activeTab === "my"}
            onClick={() => setActiveTab("my")}
          />
          <Tab label={useLocale({label: "imagegen.allPublicArtworks"})} isActive={activeTab === "public"} onClick={() => setActiveTab("public")} />
        </div>
      </div>
