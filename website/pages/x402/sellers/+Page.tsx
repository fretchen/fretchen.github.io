...
return new Response(JSON.stringify(result), { status: 200 });`}</CodeBlock>
  ...
  "extra": { "name": "USDC", "version": "2" }
          ...
          <p>
            ...
            <code>exact</code> later, is not deployed on OP Sepolia — use Base Sepolia to rehearse that one. It draws on
            the same fee allowance described below, so nothing extra needs approving to use it, and there is no separate
            recipient whitelist to get onto.
          </p>
          ...
          <p>
            ...
            free, off-chain signatures, and the fee is charged once when you claim the accumulated batch — which is what
            makes the channel scheme cheap per request in the first place.
          </p>
