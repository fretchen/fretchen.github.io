...
- **Buyer-pays split** (`x402_splitter_*.js`, `EIP3009SplitterV1`) — retired, retained but not deployed or built. The buyer signs a single EIP-3009 authorization to a splitter contract, which atomically pays seller and facilitator. The seller needs no setup, but the buyer needs a non-stock client, and `payTo` shows the splitter rather than the actual recipient — so the buyer cannot see who they are paying from the payment requirements alone. The splitter (and its own tests, `test/x402_splitter_*.test.js`) is also the only intentional JS left in this package — every other module and its live-code tests are TypeScript under `strict: true`; `test/no_stray_js.test.ts` fails if a new `.js` file shows up outside this exception plus the root `*.config.js` files.
